...
$spike = $PSScriptRoot
$repo = Split-Path $spike -Parent
$artifacts = Join-Path $spike "artifacts"
...
$consumer = Join-Path $spike "Consumer\Consumer.csproj"
$azureConsumer = Join-Path $spike "AzureConsumer\AzureConsumer.csproj"
...
foreach ($generatedPath in @(
    (Join-Path $spike "Consumer\bin"),
    (Join-Path $spike "Consumer\obj"),
    (Join-Path $spike "AzureConsumer\bin"),
    (Join-Path $spike "AzureConsumer\obj")
)) {
...
$consumerAssets = Get-Content (Join-Path $spike "Consumer\obj\project.assets.json") -Raw |
    ConvertFrom-Json
...
$azureConsumerAssets = Get-Content (Join-Path $spike "AzureConsumer\obj\project.assets.json") -Raw |
    ConvertFrom-Json
...
$azureConsumerOutput = Join-Path $spike "AzureConsumer\bin\Release\net10.0"
